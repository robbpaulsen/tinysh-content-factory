...
class VideoBuilder:
    ...
    def set_background_image(self, file_path: str, effect_config: dict = None):
        ...
        self.background = {
            ...
            or {"effect": "ken_burns"},  # Default to Ken Burns for backward compatibility
        }
        return self
    ...
    def build_command(self):
        ...
        if self.background["type"] == "video" and not self.audio_file and self.captions is None:
        ...
        if self.background["type"] == "image":
            cmd.extend(["-loop", "1", "-t", str(audio_duration), "-i", self.background["file"]])
