...
class Settings(BaseSettings):
    ...
    media_server_url: str = Field(
        default="http://localhost:8000", description="Local media processing server URL"
    )

    # Workflow Configuration
    subreddit: str = Field(default="selfimprovement", description="Reddit subreddit to scrape")
    content_type: str = Field(
        default="motivational speech", description="Type of content to generate"
    )
    art_style: str = Field(
    ...
    profiles_path: Path = Field(
        default=Path("config/profiles.yaml"), description="Path to profiles configuration file"
    )

    # Image Generation
    flux_model: str = Field(
        default="black-forest-labs/FLUX.1-schnell-Free", description="FLUX model to use"
    )
    ...
    youtube_category_id: str = Field(
        default="22", description="YouTube video category (22=People & Blogs)"
    )
    youtube_timezone: str = Field(
        default="America/Mexico_City",
        description="Timezone for scheduling (e.g., America/Mexico_City, America/New_York)",
    )
    ...
    youtube_schedule_interval_hours: int = Field(
        default=2, ge=1, le=12, description="Hours between video publishes (default: 2)"
    )
    ...
    media_processing_timeout: float = Field(
        default=300.0, description="Media processing timeout in seconds"
    )

    # FFmpeg Optimization
    ffmpeg_encoder: Literal["nvenc", "x264", "auto"] = Field(
        default="auto", description="Video encoder (nvenc=GPU, x264=CPU, auto=detect)"
    )
    ...
    ffmpeg_cq: int = Field(
        default=23, ge=18, le=28, description="Video quality (18=best, 28=worst)"
    )
    ffmpeg_bitrate: str = Field(default="5M", description="Target video bitrate (e.g., 5M, 8M)")
    ffmpeg_audio_bitrate: str = Field(
        default="128k", description="Audio bitrate (e.g., 128k, 192k)"
    )

    # SEO Optimization
    seo_enabled: bool = Field(
        default=True, description="Enable SEO metadata generation with Gemini"
    )

    # Logging
    log_to_file: bool = Field(default=True, description="Enable logging to file with rotation")
    log_dir: Path = Field(default=Path("output/logs"), description="Directory for log files")
    log_max_age_days: int = Field(
        default=7, ge=1, le=30, description="Maximum age of log files to keep (days)"
    )
