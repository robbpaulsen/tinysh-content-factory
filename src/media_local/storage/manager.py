"""Media storage manager with security validation - Ported from server/video/storage.py

Provides secure file storage with path traversal protection and media type validation.
"""
...
class MediaType:
    """Media type constants for storage organization."""
    IMAGE = "image"
    VIDEO = "video"
    AUDIO = "audio"
    TMP = "tmp"
...
class StorageManager:
    """Secure media storage manager with path validation."""
    ...
    def __init__(self, storage_path: str):
        ...
        Args:
            storage_path: Root directory for media storage
        """
    ...
    def upload_media(
        self, media_type: str, media_data: bytes, file_extension: str = ""
    ) -> str:
        """
        Uploads media to the storage.
    ...
    def create_media_filename(
        ...
        """
        Creates a unique media filename without saving to disk.

        Args:
            media_type: Type of media (image, video, audio, tmp)
    ...
    def create_media_filename_with_id(
        ...
        """
        Creates a media filename and returns both ID and full path.
    ...
    def create_media_template(
        ...
        """
        Creates a media template filename for ffmpeg output patterns.

        Args:
            media_type: Type of media, e.g., MediaType.IMAGE.
            file_extension: File extension, e.g., '.jpg', '.mp4'.

        Returns:
            Tuple of (filename, file_path) with %02d placeholder for frame numbering
        """
    ...
    def create_tmp_file(self, media_id: str) -> str:
        """
        Creates a temporary file for media upload tracking.
...
__all__ = ["StorageManager", "MediaType"]
