"""Kokoro TTS engine - Ported from server/video/tts.py

Provides multilingual text-to-speech using the Kokoro-82M model.
Supports English, Spanish, French, Hindi, Italian, Portuguese, Japanese, and Chinese.
"""
...
from src.media_local.config import device
...
class KokoroTTS:
    """Multilingual TTS using Kokoro-82M model."""
    ...
    def kokoro_english(
        ...
        pipeline = KPipeline(lang_code=lang_code, repo_id="hexgrad/Kokoro-82M", device=device_str)
    ...
    def generate(
        ...
        """Main generation method - auto-detects language from voice."""
        if not text or not text.strip():
            raise ValueError("Text cannot be empty or whitespace")
        lang_code = LANGUAGE_VOICE_MAP.get(voice, {}).get("lang_code")
        if not lang_code:
    ...
    def valid_voices(self, lang_code=None) -> List[str]:
        """
...
__all__ = ["KokoroTTS", "LANGUAGE_CONFIG", "LANGUAGE_VOICE_CONFIG", "LANGUAGE_VOICE_MAP"]
