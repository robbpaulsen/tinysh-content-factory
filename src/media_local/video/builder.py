"""Video Builder - Fluent interface for FFmpeg video construction

Ported from server/video/builder.py
"""
...
class VideoBuilder:
    ...
    def set_background_image(self, file_path: str, effect_config: dict = None):
        ...
        self.background = {
            ...
            or {
                "effect": "ken_burns"
            },  # Default to Ken Burns for backward compatibility
        }
        return self
    ...
    def build_command(self):
        ...
        if self.background["type"] == "image":
            ...
            effect_config = self.background.get(
                "effect_config", {"effect": "ken_burns"}
            )

            # Handle backward compatibility for old ken_burns config
            if (
                "ken_burns" in self.background
                and "effect_config" not in self.background
            ):
            ...
            if effect_type == "ken_burns":
                ...
                zoom_expr = zoom_expressions.get(
                    direction, zoom_expressions["zoom-to-top-left"]
                )
                ...
                pan_x_expr = (
                    f"{start_x}+({end_x}-{start_x})*t/{audio_duration}*{speed_mult}"
                )
                pan_y_expr = (
                    f"{start_y}+({end_y}-{start_y})*t/{audio_duration}*{speed_mult}"
                )
                filter_parts.append(
