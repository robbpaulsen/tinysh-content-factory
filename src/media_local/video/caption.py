"""Caption and subtitle generation - Ported from server/video/caption.py

Provides intelligent text segmentation for subtitles with support for multiple languages
and ASS subtitle file generation with customizable styling.
"""
...
class Caption:
    """Subtitle generation and text segmentation engine."""

    def is_punctuation(self, text):
        """Check if text is a punctuation character."""
    ...
    def create_subtitle(
        ...
        Args:
            segments: List of subtitle segments with text, start_ts, end_ts
            dimensions: Tuple of (width, height) for video
            output_path: Path to save ASS file
            font_size: Font size in pixels
            font_color: Hex color for text
            shadow_color: Hex color for drop shadow
            shadow_transparency: Shadow transparency (0.0=opaque, 1.0=transparent)
            shadow_blur: Blur amount for shadow
            stroke_color: Hex color for text outline
            stroke_size: Outline width
            font_name: Font family name
            font_bold: Bold text
            font_italic: Italic text
